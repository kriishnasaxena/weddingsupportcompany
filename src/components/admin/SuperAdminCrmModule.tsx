...
interface SuperAdminCrmModuleProps {
...
export default function SuperAdminCrmModule({
  onNavigateToQuoteBuilder,
}: SuperAdminCrmModuleProps) {
  // View Modes: 'kanban' | 'table' | 'partner_performance'
  const [viewMode, setViewMode] = useState<'kanban' | 'table' | 'partner_performance'>('kanban');

  // Master Resort List for Admin Filter
  const [allResorts, setAllResorts] = useState<ResortOption[]>([]);
  const [selectedResortFilter, setSelectedResortFilter] = useState<string>('ALL');
    ...
    const fetchResortsList = async () => {
      try {
        ...
        snap.forEach((d) => {
          ...
          list.push({
            id: d.id,
            name: data._recordName || data.core_name || data.name || 'Resort',
        ...
        console.error('Error fetching resorts list for admin:', e);
      }
    };
    fetchResortsList();
  }, []);

  // --- FETCH ALL LEADS GLOBALLY ---
    ...
    try {
      ...
      snap.forEach((d) => {
        ...
        list.push({
          ...
          resortName: data.resortName || 'Resort Property',
      ...
      console.error('Error fetching global CRM leads:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  // Global Platform Growth Analytics
  const globalAnalytics = useMemo(() => {
    const relevantLeads =
      selectedResortFilter === 'ALL'
        ? leads
        : leads.filter((l) => l.resortId === selectedResortFilter);

    const totalLeads = relevantLeads.length;
    const newLeads = relevantLeads.filter((l) => l.stage === 'New').length;
    const quotesSent = relevantLeads.filter((l) => l.stage === 'Quote Sent' || l.stage === 'Negotiation').length;
    const wonBookings = relevantLeads.filter((l) => l.stage === 'Won');
  ...
  }, [leads, selectedResortFilter]);

  // Partner Performance Matrix
  const partnerPerformanceMatrix = useMemo(() => {
    const map = new Map<string, { resortName: string; total: number; answered: number; won: number }>();

    leads.forEach((l) => {
      const key = l.resortName.trim() || 'Unknown Resort';
      if (!map.has(key)) {
        map.set(key, { resortName: key, total: 0, answered: 0, won: 0 });
      }
      const entry = map.get(key)!;
      entry.total++;
      if (l.stage !== 'New') entry.answered++;
  ...
  const filteredLeads = useMemo(() => {
    return leads.filter((l) => {
      const matchesResort = selectedResortFilter === 'ALL' || l.resortId === selectedResortFilter;
      ...
      return matchesResort && matchesSearch && matchesStage;
    });
  }, [leads, searchQuery, stageFilter, selectedResortFilter]);

  // Stage Updater
  const handleUpdateStage = async (leadId: string, newStage: CrmLead['stage']) => {
    if (newStage === 'Quote Sent') {
      alert(
        "Notice: 'Quote Sent' stage is updated automatically when you click 'Send Offer to Customer' inside the Quote Builder!"
      );
      return;
    }
  ...
  // TRIGGER QUOTE BUILDER REDIRECTION (FIXED: STORES TARGET INQUIRY ID IN LOCALSTORAGE)
  const handleCreateQuoteForLead = (lead: CrmLead) => {
    setDrawerOpen(false);

    // 1. Store target inquiry ID for the Quote Builder
    ...
    // 3. Switch active tab in parent dashboard
  ...
  return (
    <div className="space-y-6 w-full max-w-full min-w-0 font-sans overflow-x-hidden">
      {/* HEADER BANNER */}
      <div className="bg-white border border-gray-200 rounded-2xl p-4 shadow-xs flex flex-col sm:flex-row justify-between sm:items-center gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-base font-black text-gray-900">👑 Master Sales Lead CRM (Super Admin)</h2>
            <p className="text-xs text-gray-500 font-medium">
              Global sales pipelines, resort partner performance tracker, and lead management.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          <div className="flex gap-1 bg-gray-100 p-1 rounded-xl">
            ...
            <button
              onClick={() => setViewMode('partner_performance')}
              className={`px-3 py-1.5 rounded-lg text-xs font-bold transition cursor-pointer ${
                viewMode === 'partner_performance' ? 'bg-[#6B0D24] text-white shadow-xs' : 'text-gray-600'
              }`}
            >
      ...
      {/* SUPER ADMIN GLOBAL RESORT FILTER */}
      <div className="bg-stone-50 border border-stone-200 p-3.5 rounded-2xl flex flex-col sm:flex-row justify-between sm:items-center gap-3">
        <div className="flex items-center gap-2">
          <span className="bg-[#6B0D24] text-white text-[10px] font-black uppercase px-2.5 py-1 rounded-md">
            👑 Global Property Filter
          </span>
          <span className="text-xs font-bold text-gray-800">Filter platform lead pipeline by resort:</span>
        </div>

        <select
          value={selectedResortFilter}
          onChange={(e) => setSelectedResortFilter(e.target.value)}
          className="bg-white border border-gray-300 font-bold text-xs text-gray-900 rounded-xl px-3 py-2 outline-none cursor-pointer"
        >
          <option value="ALL">-- All Resorts ({leads.length} Total Leads) --</option>
          {allResorts.map((r) => (
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="bg-blue-50/70 border border-blue-100 p-4 rounded-2xl">
          ...
          <p className="text-2xl font-black text-blue-950 mt-1">{globalAnalytics.totalLeads}</p>
          <span className="text-[10px] font-bold text-blue-700">{globalAnalytics.newLeads} Uncontacted New</span>
        </div>
        ...
        <div className="bg-amber-50/70 border border-amber-100 p-4 rounded-2xl">
          ...
          <p className="text-2xl font-black text-amber-950 mt-1">{globalAnalytics.quotesSent}</p>
        ...
        <div className="bg-green-50/70 border border-green-100 p-4 rounded-2xl">
          ...
          <p className="text-2xl font-black text-green-950 mt-1">{globalAnalytics.conversionRate}%</p>
          <span className="text-[10px] font-bold text-green-800">{globalAnalytics.wonCount} Bookings Won</span>
        </div>
        ...
        <div className="bg-purple-50/70 border border-purple-100 p-4 rounded-2xl">
          ...
          <p className="text-2xl font-black text-purple-950 mt-1">₹{formatCurrency(globalAnalytics.wonValue)}</p>
      ...
      {/* MAIN VIEW AREA (PREVENTS HORIZONTAL SCREEN STRETCHING) */}
      <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-xs w-full max-w-full min-w-0 overflow-x-hidden min-h-[400px]">
        {loading ? (
          <div className="py-20 text-center text-gray-400 text-xs font-bold uppercase tracking-wider">
            ...
            Syncing Master CRM pipeline...
          </div>
        ) : viewMode === 'partner_performance' ? (
          /* ================= MODE 3: RESORT PARTNER RESPONSE MATRIX ================= */
          <div className="space-y-4">
            <h3 className="text-base font-black text-gray-900 border-b pb-2">
              Resort Partner Response Rate &amp; Conversion Matrix
            </h3>
            ...
            <div className="overflow-x-auto border border-gray-200 rounded-2xl">
              <table className="w-full text-left text-xs">
                <thead className="bg-gray-50 border-b border-gray-200 text-gray-500 font-bold uppercase text-[10px]">
                  <tr>
                    <th className="p-3.5">Resort Property</th>
                    <th className="p-3.5">Total Inquiries</th>
                    <th className="p-3.5">Responded / Quotes Sent</th>
                    <th className="p-3.5">Response Rate %</th>
                    <th className="p-3.5 text-right">Bookings Won</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100 font-medium text-gray-700">
                  {partnerPerformanceMatrix.map((matrix, idx) => {
                    const responseRate = matrix.total > 0 ? Math.round((matrix.answered / matrix.total) * 100) : 0;

                    return (
                      <tr key={idx} className="hover:bg-stone-50 transition">
                        <td className="p-3.5 font-bold text-gray-900">{matrix.resortName}</td>
                        <td className="p-3.5 font-mono font-bold text-blue-600">{matrix.total}</td>
                        <td className="p-3.5 font-mono font-bold text-amber-600">{matrix.answered}</td>
                        <td className="p-3.5">
                          <span
                        ...
                        <td className="p-3.5 text-right font-black text-green-700">{matrix.won}</td>
                      </tr>
                    );
                  })}
                </tbody>
          ...
          /* ================= MODE 1: KANBAN BOARD (BOUNDED HORIZONTAL SCROLL) ================= */
                    ...
                    {stageLeads.map((lead) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className="text-[10px] font-bold text-[#6B0D24] block truncate">
                            🏢 {lead.resortName}
                          </span>
                          ...
                          <button
                            ...
                            className={`w-full font-black py-1.5 rounded-lg text-[10px] uppercase tracking-wider transition shadow-2xs mt-1 cursor-pointer ${
                              isQuoteSent
                    ...
                    <tr key={lead.id} className="hover:bg-stone-50 transition">
                      ...
                      <td className="p-3.5">
                        <select
                          ...
                          className="bg-gray-50 border border-gray-200 text-xs font-bold rounded-lg p-1 outline-none cursor-pointer"
                        >
                          {allowedStages.map((s) => (
                      ...
                      <td className="p-3.5 text-right space-x-2">
                        ...
                        <button
                          ...
                          className="bg-[#6B0D24] text-white font-bold px-2.5 py-1 rounded-lg text-[10px] uppercase cursor-pointer"
                        >
                          ⚡ Revert Quote
                        </button>
                      </td>
              ...
              <div className="bg-stone-50 border border-stone-200 p-4 rounded-2xl space-y-3">
                <div className="flex justify-between items-center">
                  ...
                  <select
                    ...
                    className="bg-white border border-gray-300 font-black text-xs text-[#6B0D24] rounded-xl px-3 py-1.5 outline-none cursor-pointer"
                  >
